...
static constexpr int HYBRID1_CROSSOVER = 22;
static constexpr int HYBRID2_THRESHOLD = 22;
...
void quickSortHybrid2(Vector<int>& values, int start, int finish, Pivot pivotSelector);
...
void sortPerformanceReport() {
    ...
    cout << "    N    |   QSort1   |   QSort2   |  QSort3  " << endl;
    cout << "---------+------------+------------+----------" << endl;
    ...
    for (size_t size : sequenceSizes) {
        ...
        double hybrid1 = measureRunningTime(*random, quickSortHybrid);
        double hybrid2 = measureRunningTime(*random, quickSortHybrid2);

        cout << setw(7) << size << "  |";
        cout << setw(9) << fixed << setprecision(3) << defaultQuickSort << "   |";
        cout << setw(8) << hybrid1 << "   |";
        cout << setw(8) << hybrid2 << endl;
...
void quickSortHybrid(Vector<int>& values, int start, int finish, Pivot pivotSelector) {
    if ((finish - start + 1) <= HYBRID1_CROSSOVER) {
...
void quickSortHybrid2Recursive(Vector<int>& values, int start, int finish, Pivot pivotSelector) {
    if (finish - start + 1 <= HYBRID2_THRESHOLD) {
        return;
    }
    int boundary = partition(values, start, finish, pivotSelector);
    quickSortHybrid2Recursive(values, start, boundary - 1, pivotSelector);
    quickSortHybrid2Recursive(values, boundary + 1, finish, pivotSelector);
}

void quickSortHybrid2(Vector<int>& values, int start, int finish, Pivot pivotSelector) {
    quickSortHybrid2Recursive(values, start, finish, pivotSelector);
    insertionSort(values, start, finish);
}
