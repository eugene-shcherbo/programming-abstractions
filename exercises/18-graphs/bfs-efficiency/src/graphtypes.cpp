...
void bfs(void (*fn)(Vertex*), Vertex* start, double& avgQueueLength, long long& totalTime) {
    Set<Vertex*> visited;
    ...
    totalTime = std::chrono::duration_cast<std::chrono::milliseconds>(stopTime - startTime).count();
    avgQueueLength = queueSize / iterations;
}

void bfsEfficient(void (*fn)(Vertex*), Vertex* start, double& avgQueueLength, long long& totalTime) {
    Set<Vertex*> visited;
    Queue<Vertex*> toVisit;
    toVisit.enqueue(start);
    int queueSize = 0;
    ...
    auto stopTime = std::chrono::high_resolution_clock::now();
    totalTime = std::chrono::duration_cast<std::chrono::microseconds>(stopTime - startTime).count();
    avgQueueLength = queueSize / iterations;
}
